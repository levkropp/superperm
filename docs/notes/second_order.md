---
layout: math
title: "Second-order results"
---

# Second-order results

Everything here is evaluated by [`code/lemmas.py`](https://github.com/levkropp/superperm/blob/main/code/lemmas.py) against
...
`v ≤ S + q` with `comps ≥ q`. Full proof in [`pbound.md`](pbound) §8.
...
[`code/pbound.py`](https://github.com/levkropp/superperm/blob/main/code/pbound.py):
...
ceiling; it was missing a term. See [`notes/pbound.md`](pbound).
...
| IN5 | `B ≥ comps` (ordering-free) | **[REF]** — see §F and [`ordering.md`](ordering) §3; repaired to `B + σ2 ≥ comps` | 177/177 | 108/108 |
| A2/b/c | `comps ≥ v−S` ⟺ `T ≥ v+Y` ⟺ `dirty ≤ S+N−v` | **[THM]** — [`pbound.md`](pbound) §8 | 177/177 | 108/108 |
...
[`code/champions6.py`](https://github.com/levkropp/superperm/blob/main/code/champions6.py) measures all **43,096**.
...
> with `B = 1, comps = 2`. See [`notes/ordering.md`](ordering) §3. The proof
...
1. Egan's `F = 0` is *forced*, not a design choice — and since the Exposure
   ...
   [`a1_argument.md`](a1_argument): champions evade the chain tax by
...
3. `A1EQ` ("A = 1 ⟺ the Egan vertex") is settled, and negatively. The ⟹
   ...
   B = 2`. See [`notes/ordering.md`](ordering). What survives is the ⟸
...
that works counts the quotient's cycle rank instead —* [`pbound.md`](pbound)
*§8.*
