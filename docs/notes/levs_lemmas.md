...
| **3** | **Pentad Lemma** — at most five complete traversals chain; the `v = 120` rung is settled at length ≥ 5908 | **[THM]** + **[EXH]** |
...
**Update — the rung is now settled outright.** `EGAN1P`
([`pbound.md`](pbound) §13d) gives `v = 120` ⟹ length ≥ **5908** = Egan(7),
with the same one-model settlement at n = 6 (873) and n = 8 (46205):
the exact-cover rung is excluded for champions at all three. `PENTCAP` (§16)
supplies the combinatorial reason — weight-4-linked class-disjoint Pentads
cap at `n−3`, not `(n−3)!`. The 5895 above stays as the elementary route to
the same rung, and the lemma itself is load-bearing across the arsenal.
...
| result | note | code |
|---|---|---|
| Split Identity | [`split_identity.md`](split_identity) | `census.py`, `blockcount.py` |
| A-cost law | [`a_cost_law.md`](a_cost_law), [`pbound.md`](pbound) §9d | `lemmas.py` (`SLOT`) |
| Pentad Lemma | [`pentad_lemma.md`](pentad_lemma), [`lemma_arsenal.md`](lemma_arsenal) §3.5 | `rigidity7.py`, `pentad_orbits.py` |
| No split-free 872 | [`block_count_lemma.md`](block_count_lemma) §12g | `splitfree6b/c/d.c`, `data/e28_certificate.txt` |
| `A2` | [`pbound.md`](pbound) §8 | `a2hall.py`, `a2case3.py`, `lemmas.py` |
