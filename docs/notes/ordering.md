---
layout: math
title: "The ordering, and the two theorems it broke"
---
...
built by [`code/inflate.py`](https://github.com/levkropp/superperm/blob/main/code/inflate.py).
...
as the exclusion test in [`a1_argument.md`](a1_argument) §4; that section's
...
| `A2b` (`T ≥ v + Y`) | — | lower bound on `T` | statement safe under `INFL`; its old derivation went through `IN5` and via `IN5b` gave only `T ≥ v + Y − σ2`. Now **[THM]** outright — `A2` is proved in [`pbound.md`](pbound) §8, and `SIG2X` puts `σ2 = 0` at the optimum |
