...
CLAIMS = [
    ...
    ("CORECAP", "[THM]",
     ...
     "PROVED at general n (was [EXH] at n = 5, 6, 7)",
     "The elements are explicit (verified numerically at n = 5..10): "
     "a = (1 2 ... n-1), b: i -> i+2 (i <= n-3), n-2 -> 2, n-1 -> 1, n -> n, "
     "so s = a^(n-2)b = (1 2 ... n-2) -- ord(s) = n-2 is a one-line "
     "computation -- and u = a^(n-3)b has u^2 = e (four-case: b a^(n-3) b = "
     "a^2).  Then s = a.u gives the two identities s.u = a and "
     "u.s^(n-3) = a^(-1).\n"
     "THE WORD GAME.  A core chain's blocks have length n-1 (s-step) or "
     "n-2 (u-step), never two u's in a row (FORCE).  Three mechanisms: "
     "M1 (s.u must be terminal: after s,u the position is (block m-1's "
     "start).a, burned by block m-1 at power 1), M3' (s^(n-2) = e returns "
     "to start; any extension's first class is burned by block 1), "
     "M2 (after prefix u.s^(n-3) the position is a^(-1); the next block's "
     "second generator is the start's class, burned by block 1).  Every "
     "valid word is then one of S^a (a <= n-2), S^a.u (a <= n-3), "
     "u.S^a (a <= n-3), u.S^a.u (a <= n-4), and each boundary extension "
     "dies by M1/M2/M3' -- so at most n-2 blocks, attained by the Pentad "
     "chain S^(n-2).  Full write-up: docs/notes/large_n_lower_bound.md "
     "section 4b.\n"
     "Exhaustive evidence agrees: cap n-2 computed at n = 5-8 (witnesses "
     "[3,4,3], [4,5,5,4], [5,6,6,6,5], [6,7,7,7,7,6]) and the four-shape "
     "census at n = 5, 6, 7 (10/14/18 words, no fourth mechanism).\n"
     "GAP, stated: this covers chains of SINGLE BLOCKS.  A component with "
